package com.icomp.common.service.impl.icomp.v01b09.s007;
 ...
 */
public class ICOMPV01B09S007ServiceImpl extends BaseService implements ICOMPV01B09S007Service {
	...
	public Map<String, Object> getList(Outsidefactory entity) {
		...
		try {
			List<Outsidefactory> list =  outsidefactoryDao.searchByPrimaryView(entity);
			...
			if (list.size() <= 0) {
				...
				int total = outsidefactoryDao.searchByPrimaryViewCount(entity);
		...
		if (checkType == 1){
			...
			if (StringUtils.isEmpty(param.get("DIVmerchantsCode").toString())) {
				...
				try {
					...
					if (mList.size()>0){
						map.put("DIVmerchantsCode", MessageReSource.getMessageBox(
								IConstant.MARSGLS0007, langCode, langValue));
					}else {
	...
	public Map<String, Object> getOutList(Outsidefactory entity){
		...
		Map<String, Object> rtn = new HashMap<String, Object>();
		try {
			List<Outsidefactory> list = (List<Outsidefactory>) outsidefactoryDao.getDetailList(entity);
	...
	@Override
	public Map<String, Object> outsidefactoryEdid2(Outsidefactory outsidefactory, String langCode, String langValue) throws ParseException {
		Map<String, Object> rtn = new HashMap<String, Object>();
		Outsidefactory entity = null;
		int reVal = 0;
		try {
			reVal = outsidefactoryDao.updateNonQuery2(outsidefactory);
			if (reVal > 0) {
				rtn.put("message", MessageReSource.getMessageBox(
						IConstant.IMSG0005, langCode, langValue));
				rtn.put("status", IConstant.RESULT_CODE_0);
	...
	public Map<String, Object> getOutsideFactoryList(Outsidefactory outsidefactory) {
		...
		Map<String, Object> rtn = new HashMap<String, Object>();
		try {
			List<Outsidefactory> list = (List<Outsidefactory> )outsidefactoryDao.searchOutsideFactoryList(outsidefactory);
			List<Outsidefactory> list2 = (List<Outsidefactory> )outsidefactoryDao.searchAllOutsideFactoryList(outsidefactory);
			rtn.put("data", list);
			rtn.put("data2", list2);
		} catch (SQLException e) {
			e.printStackTrace();
	...
	// 2017/09/12 宋健 追加↓↓↓　dazhong@YMSC
	@Override
	public Map<String, Object> getRfidcontainerList(Rfidcontainer rf) {
		Map<String, Object> rtn = new HashMap<String, Object>();
		try {
			List<Rfidcontainer> list = rfidcontainerDao.searchByList2(rf);
			rtn.put("data", list);
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	@Override
	public Synthesisknife getSynByRfidConner(Synthesisknife skentity) throws Exception {
		// 2017/09/12 王冉 变更↓↓↓　dazhong@YMSC

		// 根据刀具ID查询材料号
		Synthesisknife reEntity =  synthesisknifeDao.searchSyn(skentity);
		return reEntity;
		// 2017/09/12 王冉 变更↑↑↑　dazhong@YMSC
	}
	...
	public Tool searchToolInfoBySpcode(String spCode) {
		List<Tool> list =null;
		try {
			list = toolDao.searchToolInfoBySpcode(spCode);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list.get(0);
	...
	@Override
	public int updateSynthesisknife(Synthesisknife sk) {
		int r = 0;
		try {
			// 2017/09/12 王冉 变更↓↓↓　dazhong@YMSC
			r = synthesisknifeDao.updateNonQuery(sk);
			// 2017/09/12 王冉 变更↑↑↑　dazhong@YMSC
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	@Override
	public int insertAuthorizationDao(Authorization authorization) {
		int r = 0;
		try {
	...
	@Override
	public List<Tooltransfer> searchTooltransferList(Tooltransfer t) {
		List<Tooltransfer> list =null;
		try {
			list = tooltransferDao.searchTooltransferList(t);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
	...
	@Override
	public int updateTooltransfer(Tooltransfer t) {
		int r = 0;
		try {
			// 2017/09/12 王冉 变更↓↓↓　dazhong@YMSC
			r = tooltransferDao.updateNonQuery(t);
			// 2017/09/12 王冉 变更↑↑↑　dazhong@YMSC
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	@Override
	public TooltransferTotal getTooltransferTotalInfoByToolCode(TooltransferTotal t) {
		List<TooltransferTotal> list = null;
		TooltransferTotal tooltransferTotal = null;
		try {
			list = (List<TooltransferTotal>)tooltransferDao.searchList(t);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		if(list.size() > 0){
			tooltransferTotal = new TooltransferTotal();
			tooltransferTotal =  list.get(0);
		}
		return tooltransferTotal;
	}

	@Override
	public int updateTooltransferTotalInfo(TooltransferTotal t) {
		int r = 0;
		try {
			r = tooltransferDao.updateTooltransferTotalInfo(t);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return r;
	...
	public Tool searchBitInputInf(Tool entity) {
		List<Tool> list = null;
		try {
			list = toolDao.searchBitInputInfo(entity);
		} catch (SQLException e) {
			e.printStackTrace();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Rfidcontainer getRfidInfoByRfid(String rfId) {
		...
		try {
			list = rfidcontainerDao.getRfidInfoByRfid(rfId);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		if (list == null) {
