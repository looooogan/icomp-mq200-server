...
public class EvalUtils {
    public static String eval(String exp) {
        try {
            ...
            System.out.println("EvalUtils" + e.toString());
            return IConstant.DEL_FLAG_0;
        }
    }

    // 遇到操作符压栈，遇到表达式从后缀表达式中弹出两个数，计算出结果，压入堆栈
    private static String doEval(List list) throws Exception {
    ...
    private static List infixExpToPostExp(String exp) throws Exception {// 将中缀表达式转化成为后缀表达式
        exp = exp + "#";
