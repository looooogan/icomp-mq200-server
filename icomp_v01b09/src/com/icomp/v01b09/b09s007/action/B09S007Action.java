package com.icomp.v01b09.b09s007.action;
 ...
 */
public class B09S007Action extends IcompAction {
	 ...
	 * 修复通知单查询Business
	 */
	private B09S007Business b09s007Business;

	public void setB09s007Business(B09S007Business b09s007Business) {
		this.b09s007Business = b09s007Business;
	}
	...
	public String initb09S007() {
		try {
			...
			if ("list".equals ( param.get ( "opt" ) )) {
				//  修复通知单查询列表
				Map<String, Object> list = this.b09s007Business.getList ( param, langCode, langValue, 1 );
				...
				//取得下拉列表列表
				Map<String, Object> list = this.b09s007Business.getPageSelList ( langCode, langValue );
				super.assign ( "MerchantsesList", list.get ( "merchantsesList" ) );
				// 取得页面grid显示项目列表
				super.session ( "gridcol", b09s007Business.getGridColmun ( "B03S002", ((Customer) super.session ( "Customer" )).getCustomerID (), langCode, langValue ) );
				return SUCCESS;
			}
		} catch (BusinessException ex) {
			ApplicationException.setApplicationException ( getResponse (), "/B09S007.action", IConstant.RESULT_CODE_1, ex, this.getClass ().getName (), "B09S007", ((Customer) super.session ( "Customer" )).getCustomerID () );
			return null;
		}
	}
	...
	public String outFactoryAdd() {
		try {
			...
			if ("add".equals ( param.get ( "opt" ) )) {
				...
				String customerId = ((Customer) super.session("Customer")).getCustomerID();
				//  修复通知单查询列表
				Map<String, Object> list = this.b09s007Business.outFactoryAdd ( param, customerId, langCode, langValue );
				// 2017/09/12 宋健 变更↑↑↑　dazhong@YMSC
				super.ajaxReturn ( list );
				return null;
			}
			// 取得页面grid显示项目列表
			super.session ( "gridcol", b09s007Business.getGridColmun ( "B09S007", ((Customer) super.session ( "Customer" )).getCustomerID (), langCode, langValue ) );
			return SUCCESS;
		} catch (BusinessException ex) {
			ApplicationException.setApplicationException ( getResponse (), "/B09S007.action", IConstant.RESULT_CODE_1, ex, this.getClass ().getName (), "B09S007", ((Customer) super.session ( "Customer" )).getCustomerID () );
			return null;
	...
	public String outInfo() {
		try {
			...
			Map<String, Object> param = super.param ();
			//			查询待编辑内容
			Map<String, Object> ret = this.b09s007Business.outInfo ( param, langCode, langValue );
	...
	public String outFactoryEdit() throws ParseException {
		try {
 			String langValue = ((Languagetable) super.session ( "Languagetable" )).getLanguageValue ();
			String langCode = ((Languagetable) super.session ( "Languagetable" )).getLanguageCode ();
			Map<String, Object> param = super.param ();
			...
			if ("editOutDesc".equals ( param.get ( "opt" ) )) {
				list = this.b09s007Business.outFactoryEdit ( param, langCode, langValue, customerId);
			}else{
				list = this.b09s007Business.outFactoryEdit2 ( param, langCode, langValue, customerId);
			}
	...
	public String outFactoryDel() {
		try {
			String langValue = ((Languagetable) super.session ( "Languagetable" )).getLanguageValue ();
			String langCode = ((Languagetable) super.session ( "Languagetable" )).getLanguageCode ();
			Map<String, Object> param = super.param ();
			//	查询待编辑内容
			Map<String, Object> list = this.b09s007Business.outFactoryDel ( param, langCode, langValue, ((Customer) super.session ( "Customer" )).getCustomerID () );
			super.ajaxReturn ( list );
			return null;
		} catch (BusinessException ex) {
			ApplicationException.setApplicationException ( getResponse (), "/userInfo.action", IConstant.RESULT_CODE_1, ex, this.getClass ().getName (), "userInfo", ((Customer) super.session ( "Customer" )).getCustomerID () );
	...
	public String exportExcel() {
		...
		Map<String, Object> outsideFactoryList = new HashMap<String, Object> ();
		outsideFactoryList = this.b09s007Business.getOutsideFactoryList ( entity );
		...
		try {
			...
			sheet.setColumnWidth(1, 14 * 256);  //设置列宽
			sheet.setColumnWidth(2, 14 * 256);  //设置列宽
			sheet.setColumnWidth(3, 14 * 256);  //设置列宽
			sheet.setColumnWidth(4, 14 * 256);  //设置列宽
			sheet.setColumnWidth(5, 14 * 256);  //设置列宽
			sheet.setColumnWidth(6, 14 * 256);  //设置列宽
			sheet.setColumnWidth(7, 14 * 256);  //设置列宽
			sheet.setColumnWidth(8, 14 * 256);  //设置列宽
			sheet.setColumnWidth(9, 14 * 256);  //设置列宽
			sheet.setColumnWidth(10, 14 * 256);  //设置列宽
			sheet.addMergedRegion ( new CellRangeAddress ( 0, 0, 3, 9 ) );//合并单元格
			...
			style2.setFont(font);
			style2.setFillForegroundColor((short) 40);
			style2.setFillPattern(CellStyle.SOLID_FOREGROUND);

			HSSFRow rowss = sheet.createRow ( 0 );//创建第一行
			HSSFCell cellss = rowss.createCell ( 3 );
			cellss.setCellStyle ( style1 );
			cellss.setCellValue ( "传动器车间MQ200区域刀具外委涂层/修磨收发单" );

			HSSFRow rows = sheet.createRow ( 2 );//创建第二行
			sheet.addMergedRegion ( new CellRangeAddress ( 1, 1, 0, 0 ) );//合并单元格
			HSSFCell cells = rows.createCell ( 2 );//创建这行的第一个元素.从0开始
			cells.setCellStyle ( style2 );
			cells.setCellValue ( "序号" );

			cells = rows.createCell ( 3 );
			cells.setCellStyle ( style2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 1, 1, 1, 1 ) );//合并单元格
			cells.setCellValue ( "材料号" );

			cells = rows.createCell ( 4 );
			cells.setCellStyle ( style2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 1, 1, 2, 2 ) );//合并单元格
			cells.setCellValue ( "名称" );

			cells = rows.createCell ( 5 );
			cells.setCellStyle ( style2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 1, 1, 3, 3 ) );//合并单元格
			cells.setCellValue ( "描述" );

			cells = rows.createCell ( 6 );
			cells.setCellStyle ( style2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 1, 1, 4, 4 ) );//合并单元格
			cells.setCellValue ( "刀具管理号" );

			cells = rows.createCell ( 7 );
			cells.setCellStyle ( style2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 1, 1, 5, 5 ) );//合并单元格
			cells.setCellValue ( "发出数量" );

			cells = rows.createCell ( 8 );
			cells.setCellStyle ( style2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 1, 1, 6, 6 ) );//合并单元格
			cells.setCellValue ( "累计返厂数量" );

			cells = rows.createCell ( 9 );
			cells.setCellStyle ( style2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 1, 1, 3, 3 ) );//合并单元格
			cells.setCellValue ( "最后返厂数量" );
			...
			int numberGrindingTotal = 0;
			int receiveNumberTotal = 0;
			int lastBackNumTotal = 0;
			for (int i = 0;i<list.size();i++){
				...
				cell.setCellStyle( style3 );
				cell.setCellValue(list.get(i).getMaterialNum());

				cell = row.createCell(4);
				cell.setCellStyle( style3 );
				cell.setCellValue(list.get(i).getToolName());

				cell = row.createCell(5);
				cell.setCellStyle( style3 );
				cell.setCellValue(list.get(i).getDescr());

				cell = row.createCell(6);
				cell.setCellStyle( style3 );
				cell.setCellValue(list.get(i).getLaserCode());

				cell = row.createCell(7);
				cell.setCellStyle( style3 );
				cell.setCellValue(list.get(i).getNumberGrinding());

				cell = row.createCell(8);
				cell.setCellStyle( style3 );
				cell.setCellValue(list.get(i).getReceiveNumber());

				cell = row.createCell(9);
				cell.setCellStyle( style3 );
				// 2017/09/16 宋健 变更↓↓↓　dazhong@YMSC
				if("".equals(list.get(i).getLastBackNum())){
					cell.setCellValue("0");
				}else {
					cell.setCellValue(list.get(i).getLastBackNum());
				}
				cell = row.createCell(10);
				cell.setCellStyle( style3 );
				cell.setCellValue(list.get(i).getNote());

				numberGrindingTotal += Integer.valueOf(list.get(i).getNumberGrinding());
				receiveNumberTotal += Integer.valueOf(list.get(i).getReceiveNumber());
				if(!"".equals(list.get(i).getLastBackNum())){
					lastBackNumTotal += Integer.valueOf(list.get(i).getLastBackNum());
				}
				// 2017/09/16 宋健 变更↑↑↑　dazhong@YMSC
			}

			HSSFRow row = sheet.createRow ( 3 + list.size() );//创建第二行
			HSSFCell cell = row.createCell ( 2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 3 + list.size(), 3 + list.size(), 2, 6 ) );//合并单元格
			cell.setCellStyle ( style3 );
			...
			cell = row.createCell ( 7 );
			cell.setCellStyle( style3 );
			cell.setCellValue("" + numberGrindingTotal);

			cell = row.createCell ( 8 );
			cell.setCellStyle( style3 );
			cell.setCellValue("" + receiveNumberTotal);

			cell = row.createCell ( 9 );
			cell.setCellStyle( style3 );
			cell.setCellValue("" + lastBackNumTotal);

			cell = row.createCell ( 10 );
			cell.setCellStyle( style3 );
			cell.setCellValue("");

			HSSFRow rowsss = sheet.createRow ( 5 + list.size() );
			HSSFCell cellsss = rowsss.createCell ( 2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 5 + list.size(), 5 + list.size(), 2, 3 ) );//合并单元格
			cellsss.setCellStyle ( style );
			cellsss.setCellValue ( "磨刀间负责人:" );

			cellsss = rowsss.createCell ( 9 );
			cellsss.setCellStyle ( style );
			cellsss.setCellValue ( "厂家送货人:" );

			HSSFRow rows6 = sheet.createRow ( 6 + list.size() );
			HSSFCell cells6 = rows6.createCell ( 2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 6 + list.size(), 6 + list.size(), 2, 3 ) );//合并单元格
			cells6.setCellStyle ( style );
			cells6.setCellValue ( "厂家取货人:" );

			cells6 = rows6.createCell ( 9 );
			cells6.setCellStyle ( style );
			cells6.setCellValue ( "库房接收人:" );

			HSSFRow rows7 = sheet.createRow ( 7 + list.size() );
			HSSFCell cells7 = rows7.createCell ( 2 );
			sheet.addMergedRegion ( new CellRangeAddress ( 7 + list.size(), 7 + list.size(), 2, 3 ) );//合并单元格
			cells7.setCellStyle ( style );
			cells7.setCellValue ( "取货日期:" );

			cells7 = rows7.createCell ( 9 );
			cells7.setCellStyle ( style );
			cells7.setCellValue ( "送回日期:" );
	...
	public String exportExcelPrint() {
		...
		try {
			...
			if (!StringUtils.isEmpty(grindingName)) {
				Map<String, Object> list = this.b09s007Business.getPageSelList("", "");
			...
			try {
				dataMap = this.b09s007Business.getList ( param, "", "", 2 );
			} catch (Exception e1) {
