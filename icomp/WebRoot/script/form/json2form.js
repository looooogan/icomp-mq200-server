...
$.fn.json2form = function(config ) { 
	...
	if(config.data){//input text password hidden button reset submit checkbox radio select textarea
		$("#"+config.elem+" input,select,textarea").each(function(){
			//*****************************************
			//增加表单验证，防止给其它表单上的下拉菜单赋值
			//*****************************************
			var myElem = this;
			var ifChange = false;
			getMyDefinedFormName(myElem);
			if(myFormName == config.elem) {
				ifChange = true;
			}
			//******************end*********************
...
function getMyDefinedFormName(obj) {
	if (obj.parentNode != undefined && obj.parentNode.nodeName.toLowerCase() == "form") {
		myFormName = obj.parentNode.name;
	} else {
...
function setContorllBackColor(form,message) {
	var msg='';
		  ...
		  //
	    	  ...
	    	  if($(this).attr('name')== key) 
	    		  if($(this).hasClass("u-sel")){
	    			$(this).css('border-color','#c00 #e00 #e00');
	    		  }else{
	    	        $(this).addClass('u-ipt-err');
	    		  }
	    	  });
